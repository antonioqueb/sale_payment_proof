...
<odoo>
    ...
    <record id="view_sale_payment_proof_list" model="ir.ui.view">
        ...
        <field name="model">sale.payment.proof</field>
        <field name="arch" type="xml">
            <list string="Comprobantes de Pago" decoration-success="state == 'applied'" decoration-danger="state == 'rejected'" decoration-warning="state == 'pending'">
                <field name="upload_date"/>
                <field name="name"/>
                ...
                <field name="reference"/>
                <field name="uploaded_by"/>
                <field name="state" widget="badge" decoration-success="state == 'applied'" decoration-danger="state == 'rejected'" decoration-warning="state == 'pending'"/>
            </list>
        </field>
    </record>

    <!-- Vista Kanban embedida -->
    <record id="view_sale_payment_proof_kanban" model="ir.ui.view">
        <field name="name">sale.payment.proof.kanban</field>
        <field name="model">sale.payment.proof</field>
        <field name="arch" type="xml">
            <kanban class="o_payment_proof_kanban" default_order="upload_date desc">
                <field name="id"/>
                <field name="name"/>
                <field name="amount"/>
                ...
                <templates>
                    <t t-name="kanban-box">
                        <div class="oe_kanban_global_click o_payment_proof_card">
                            <div class="o_pp_preview">
                                <t t-if="record.is_image.raw_value">
                                    <img t-attf-src="/web/image/sale.payment.proof/#{record.id.raw_value}/file"
                                         alt="Comprobante"
                                         class="o_pp_thumb"/>
                                </t>
                                ...
                                <span t-attf-class="o_pp_state badge #{record.state.raw_value == 'applied' ? 'text-bg-success' : record.state.raw_value == 'rejected' ? 'text-bg-danger' : 'text-bg-warning'}">
                                    <field name="state"/>
                                </span>
                            </div>
                            <div class="o_pp_body">
                                ...
                                <div class="o_pp_amount" t-if="record.amount.raw_value">
                                    <field name="amount" widget="monetary"/>
                                </div>
                                <div class="o_pp_meta">
                                    <div t-if="record.payment_date.raw_value">
                                        <i class="fa fa-calendar"/> <field name="payment_date"/>
                                    </div>
                                    <div t-if="record.payment_method.raw_value">
                                        <i class="fa fa-credit-card"/> <field name="payment_method"/>
                                    </div>
                                    <div t-if="record.reference.raw_value">
    ...
    <record id="view_sale_payment_proof_search" model="ir.ui.view">
        ...
        <field name="model">sale.payment.proof</field>
        <field name="arch" type="xml">
            <search string="Comprobantes de Pago">
                <field name="name"/>
                <field name="sale_order_id"/>
                <field name="partner_id"/>
                <field name="reference"/>
                ...
                <filter string="Aplicados" name="filter_applied" domain="[('state', '=', 'applied')]"/>
                <filter string="Rechazados" name="filter_rejected" domain="[('state', '=', 'rejected')]"/>
                <separator/>
                <filter string="Mis cargas" name="filter_mine" domain="[('uploaded_by', '=', uid)]"/>
                <group expand="0" string="Agrupar por">
                    <filter string="Cliente" name="group_partner" context="{'group_by': 'partner_id'}"/>
                    <filter string="Estado" name="group_state" context="{'group_by': 'state'}"/>
                    <filter string="Método de pago" name="group_method" context="{'group_by': 'payment_method'}"/>
                    <filter string="Subido por" name="group_uploader" context="{'group_by': 'uploaded_by'}"/>
                    <filter string="Fecha de carga" name="group_upload" context="{'group_by': 'upload_date'}"/>
                </group>
            </search>
        </field>
    </record>
    ...
    <record id="action_sale_payment_proof" model="ir.actions.act_window">
        ...
        <field name="search_view_id" ref="view_sale_payment_proof_search"/>
        ...
        <field name="help" type="html">
            ...
            <p>
                Los comprobantes de pago se cargan desde la pestaña <b>Comprobantes de Pago</b> en cada orden de venta.
            </p>
        </field>
    </record>
